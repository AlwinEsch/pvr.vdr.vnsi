...
namespace V2
{
  ...
  inline void CAddonGUIWindow::ClearProperty(const std::string& key)
  {
    m_cb->Window.ClearProperty(m_Handle->addonData, m_WindowHandle, key.c_str());
  }
  ...
  inline CAddonGUIListItem* CAddonGUIWindow::AddStringItem(const std::string& name, int itemPosition)
  {
    GUIHANDLE handle = m_cb->Window.AddStringItem(m_Handle->addonData, m_WindowHandle, name.c_str(), itemPosition);
    if (handle)
      return new CAddonGUIListItem(handle);
  ...
  inline void CAddonGUIWindow::RemoveItem(CAddonGUIListItem* item)
  {
    m_cb->Window.RemoveItemFile(m_Handle->addonData, m_WindowHandle, item->m_ListItemHandle);
  }

  inline CAddonGUIListItem* CAddonGUIWindow::GetListItem(int listPos)
  {
    GUIHANDLE handle = m_cb->Window.GetListItem(m_Handle->addonData, m_WindowHandle, listPos);
    if (handle)
      return new CAddonGUIListItem(handle);

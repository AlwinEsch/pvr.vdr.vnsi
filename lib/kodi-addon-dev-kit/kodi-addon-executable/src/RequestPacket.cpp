...
CRequestPacket::CRequestPacket(uint32_t topcode, KODI_API_Packets type, bool setUserDataLength, size_t userDataLength)
  : m_buffer(NULL),
    m_bufUsed(0),
    m_lengthSet(false)
{
...
void CRequestPacket::push(KODI_API_Datatype type, const void *value)
{
  ...
  case API_STRING:
  {
    ...
    if (!m_lengthSet)
    {
      uint32_t tmp = htonl(m_bufUsed - headerLength);
      memcpy(&m_buffer[userDataLenPos], &tmp, sizeof(uint32_t));
    }
    break;
  }
  case API_BOOLEAN:
  ...
  case API_LONG:
  {
    ...
    *((long*)(m_buffer+m_bufUsed)) = (*((long*)value));
    ...
    if (!m_lengthSet)
      *((uint32_t*)(m_buffer+userDataLenPos)) = htonll(m_bufUsed - headerLength);
    break;
  }
  ...
  case API_INT32_T:
  {
    ...
    *((int32_t*)(m_buffer+m_bufUsed)) = htonl(*((int32_t*)value));
    ...
    if (!m_lengthSet)
      *((int32_t*)(m_buffer+userDataLenPos)) = htonl(m_bufUsed - headerLength);
    break;
  }
  case API_INT64_T:
  {

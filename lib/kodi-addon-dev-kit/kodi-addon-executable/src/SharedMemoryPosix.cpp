...
bool CBinaryAddonSharedMemoryPosix::CreateSharedMemory()
{
  ...
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Request of interface 'Add-on to Kodi' shared memory failed with error '%i': %s",
  ...
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Connect of interface 'Add-on to Kodi' shared memory failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
   ...
   */
  if ((m_shmId_FromKodiToAddon = shmget(m_randomConnectionNumber, m_sharedMem_Size, IPC_CREAT | 0666)) < 0)
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Request of interface 'Kodi to Add-on' shared memory failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
  ...
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Connect of interface 'Kodi to Add-on' shared memory failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
...
void CBinaryAddonSharedMemoryPosix::DestroySharedMemory()
{
  if (m_sharedMem_AddonToKodi != NULL)
  {
    if (shmdt(m_sharedMem_AddonToKodi) != 0)
      KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Detach of shared memory 'Add-on to Kodi' failed with error '%i': %s",
                    errno, ErrorString(errno).c_str());
  ...
  if (m_sharedMem_KodiToAddon != NULL)
  {
    if (shmdt(m_sharedMem_KodiToAddon) != 0)
      KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: Detach of shared memory 'Kodi to Add-on' failed with error '%i': %s",
                    errno, ErrorString(errno).c_str());
...
bool CBinaryAddonSharedMemoryPosix::Lock_AddonToKodi_Addon()
{
  ...
  if (err != 0)
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: LockAddon of interface 'Add-on to Kodi' failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
...
bool CBinaryAddonSharedMemoryPosix::Lock_KodiToAddon_Addon()
{
  ...
  if (err != 0)
  {
    KodiAPI_Log(this, LOG_ERROR, "Binary AddOn: LockAddon of interface 'Kodi to Add-on' failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
